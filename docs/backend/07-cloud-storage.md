# Cloud Storage: code and data

> **Generated file: do not edit by hand.** Produced by `node tools/export-docs.js` (GitHub runs it after every push).
> Everything behind the [Cloud Storage](../../tabs/07-cloud-storage/index.html) tab in one place: how the page is put together, the full source of the code that draws it, and the full data it reads. **Load it when a question or change concerns how this tab works** (its calculations, data, filters or behaviour); wording-only edits do not need it. The visible text is in [docs/tabs/07-cloud-storage.md](../tabs/07-cloud-storage.md); where the data came from is in [research.md](research.md).

## How the page is put together

- Markup: [tabs/07-cloud-storage/index.html](../../tabs/07-cloud-storage/index.html) (473 lines), `<body data-page="cloud-storage">`
- Self-contained: static HTML with its own styles and the inline script below; tab bar from [assets/nav.js](../../assets/nav.js)
- Sections and the functions that fill them: see the [code map](../code-map.md#07-cloud-storage)

## Code

The page's content is static HTML in [index.html](../../tabs/07-cloud-storage/index.html); its text is in [docs/tabs/07-cloud-storage.md](../tabs/07-cloud-storage.md). Its inline script, in full:
...
:root{
  ...
  --flame-1:#2563EB; --flame-2:#0048FF; --accent:#1D4ED8; --accent-soft:#E6EEFF;
  --cyan:#0EA5E9; --pink:#8B5CF6;
...
@media (prefers-color-scheme:dark){
  :root:not([data-theme="light"]){
    ...
    --accent:#8AB0FF; --accent-soft:#15213D; --cyan:#38BDF8; --pink:#A78BFA;
...
.tally .t-fixed b{color:var(--pass)} .tally .t-open b{color:var(--warn)} .tally .t-pub b{color:var(--accent)}
.stack{display:flex;height:16px;border-radius:99px;overflow:hidden;background:var(--sunk)}
.stack span{display:block;height:100%}
.stack .s-fixed{background:var(--pass)} .stack .s-open{background:var(--warn)} .stack .s-pub{background:var(--accent)}
...
```css
/* Same dossier design as the All Document Reader tab in the sibling site, with the accent taken from this app's icon blue.
   It loads bar.css for the tab bar instead of site.css, and the sections row in the bar replaces a left-hand section rail. */
...
img{max-width:100%}
/* Play listing screenshots are 9:16 key art, not raw 9:20 device captures: show them whole. */
.phone.key img{aspect-ratio:540/959;object-fit:contain;border-width:0;border-radius:14px}
.strip figure{flex-basis:188px}
